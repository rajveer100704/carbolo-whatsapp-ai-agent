...
* **SQLite Concurrency & WAL mode**: Solves database write lockups (`database is locked` error) under simultaneous WhatsApp and Email requests by using a `StaticPool` (shared connection under `aiosqlite`) coupled with Write-Ahead Logging (`PRAGMA journal_mode=WAL`), `PRAGMA synchronous=NORMAL`, and a `busy_timeout` of 30,000ms.
